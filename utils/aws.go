...
func (as *awss3) ListBuckets() (res *s3.ListBucketsOutput, err error) {
	return as.S3SESSION.ListBuckets(&s3.ListBucketsInput{})
}

func (as *awss3) CreateBucket() (res *s3.CreateBucketOutput, err error) {
	res, err = as.S3SESSION.CreateBucket(&s3.CreateBucketInput{
		Bucket: aws.String(as.BUCKET_NAME),
		CreateBucketConfiguration: &s3.CreateBucketConfiguration{
			LocationConstraint: aws.String(as.REGION),
		},
	})
	...
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			...
			case s3.ErrCodeBucketAlreadyExists:
				return res, errors.New("Bucket name already in use!")
			case s3.ErrCodeBucketAlreadyOwnedByYou:
				return res, errors.New("Bucket exists and is owned by you!")
			default:
				return res, err
			}
		}
